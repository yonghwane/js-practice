...
1. ## ID 사용
`document.getElementById('??').?? = ??;` 형식으로 ID 값을 사용합니다. 
...
3. ## 화면 표시
 화면을 보이게 하려면 `display: block;`, 숨기려면 `display: none;`을 사용합니다. 
...
7. ## addEventListener 사용법
 `addEventListener('이벤트', function() { 실행할 함수 })` 형식으로 사용하면, onclick 없이도 이벤트를 처리할 수 있습니다. 

8. ## 이벤트 
클릭, 키 입력, 스크롤, 드래그 등의 동작을 이벤트라고 합니다. 이벤트는 사용자의 행동을 감시합니다. 

9. ## 콜백함수
 함수의 파라미터 자리에 또 다른 함수를 넣는 것을 콜백 함수라고 합니다. 함수를 순차적으로 진행하고 싶을 때 사용합니다. 

10. ## querySelector 사용법
 `getElementById`, `getElementsByClassName[인덱스]` 대신 CSS 셀렉터를 그대로 사용할 수 있는 `querySelector`를 주로 사용합니다. 예를 들어, `querySelector('#id')`는 id라는 ID 값을 가진 요소를 선택하고, `querySelector('.className')`는 className이라는 클래스를 가진 요소를 선택합니다. 
...
## 13. 내가 원하는 움직이는 UI 만드는 고찰

1. **시작 스타일 만들기:** CSS를 사용하여 원하는 시작 스타일을 만듭니다.
2. **최종 스타일 만들기:** CSS를 사용하여 원하는 최종 스타일을 만듭니다.
3. **스타일 변경 주문:** JavaScript를 사용하여 원하는 시점에 최종 스타일로 바뀌도록 주문을 넣습니다.
4. **Transition 추가:** 시작 스타일에 Transition을 추가하여 원활한 스타일 변화를 만듭니다. 

14. ## 이미지 슬라이드 구현방법
    13번의 내용을 토대로 1. 사진들을 쭈욱 이어붙혀넣기 -> vh를 -100%씩 감소시키는 함수를 만들고 시작지점에 Transition을 1초주고 만들면 끝 
...
1. **페이지네이션에 필요한 변수 선언 (총 7개)**
   - 페이지당 보여줄 게시물 수
   - 총 몇 개 있는지 보여줄 페이지 번호 개수
   - 게시물을 담을 리스트 변수
   - 이전, 이후 페이지로 이동할 수 있게 하는 버튼 변수
   - 초기 설정할 페이지 번호

2. **필요한 페이지 번호의 총 개수 구하기**
   - 총 표현될 페이지 개수는 총 게시물 수 / 페이지당 보여줄 게시물 수
   - 올림을 해주기 위해 `Math.ceil()` 함수 사용

3. **필요한 페이지 번호의 개수에 맞게 페이지 버튼 구성하기**
   - `setPageButtons()` 함수를 만들어서 페이지 버튼 구성
   - 숫자를 표현할 HTML 초기화
   - 반복문을 통해 필요한 페이지 번호의 개수만큼 반복하여 페이지 숫자와 버튼 만들기

4. **페이지에 해당하는 게시물을 ul 변수에 추가하기**
   - `setPageOf()` 함수를 만들어서 페이지에 해당하는 게시물을 ul에 추가
   - ul 초기화
   - 반복문을 돌려서 해당 게시글의 현재 클릭된 번호부터 보여주고 싶은 게시물의 개수만큼 반복하여 게시물 추가

5. **이전, 이후 버튼을 클릭했을 때 해당 페이지로 이동하는 함수 생성**
   - 이전 버튼을 클릭했을 때 로직: 현재 페이지가 1보다 클 때 조건문을 사용해서 현재 페이지의 변수 값을 -1하고, `setPageOf()` 함수를 호출해서 그 현재 페이지의 값을 함수로 넣어서 바꿈
   - `moveSelectedPageHighlight()` 함수를 호출해서 CSS 적용

6. **해당 기능에 알맞는 CSS 적용**

## 25. 문자와 정수, 변수 등을 결합하는 방법 팁

**꿀팁1.** 문자와 정수를 결합하면 문자를 반환합니다. 예를 들어, '가격' + 5를 하면 '가격5'가 출력됨.

**꿀팁2.** '문자' + a + '문자' 형태로 정수를 중간에 넣을 수 있다. 하지만 중간중간에 +를 넣으면 가독성이 떨어집니다.

**꿀팁3.** 따라서, 백틱(``)을 사용하여 문자열 안에 변수를 집어넣는 방법을 사용하는게 좋은데, 예를 들어, `사이에 아무거나 써두고 ${변수} 이렇게 변수를 집어넣는다`

## 26. `<select>` 사용법

`<select>`는 사용자가 여러 옵션 중 하나를 선택할 수 있게하는 선택  상자다. 예를 들어, 옷의 사이즈(95, 100, 105 등)를 선택할 수 있게 하는 등의 용도로 사용이 됨.

그러면  왜 `<input>` 대신 `<select>`를 사용하는가? `<input>`은 사용자가 기상천외한 값을 입력할 수 도 있기 때문에, 제한된 선택지 중 하나를 선택하게 하려고 `<select>`를 사용한다.
